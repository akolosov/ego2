<!-- $Id: text.php,v 1.1 2005/05/11 07:38:26 hunter Exp $ -->
	  ...
	  <? endif; ?>
 	  <form name="post" action="<?= $session->utils->makeHRURL("?module=gizmo&action=show&sub=change&id=".$session->id); ?>" method="<?= $config->default_method; ?>">
	<?php $session->plugins->execHook(cHooks::beforeShowPostForm, &$session->gizmo); ?>
	   <input size="20" type="hidden" name="unixtime" value="<?=$session->gizmo['unix_time']; ?>">
	   <table width="90%" align="center" valign="top">
	    ...
	    <tr id="header01" style="display: none; ">
	     ...
	     <td width="90%" colspan="3"><input style="width: 100%" type="text" name="name" value="<?= $session->gizmo['author_name']; ?>"></td>
	    </tr>
	    <tr id="header02" style="display: none; ">
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->email; ?>:</strong></td>
	     <td width="90%" colspan="3"><input style="width: 100%" type="text" name="email" value="<?= $session->gizmo['author_email']; ?>"></td>
	    </tr>
	    <tr id="header03" style="display: none; ">
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->subject; ?>:</strong></td>
	     <td width="90%" colspan="3"><input style="width: 100%" type="text" name="topic" value="<?= $session->gizmo['title']; ?>"></td>
	    </tr>
	<? if ($config->use_categories): ?>
	    <tr id="header04" style="display: none; ">
	     ...
	     <td width="90%" colspan="3">
	       <select name="category" style="width: 100%" value="<?= $session->gizmo['category']; ?>"><?= $session->utils->getCategoriesList($session->gizmo['category'], true); ?></select>
	     </td>
	     <input size="20" type="hidden" name="oldcategory" value="<?= $session->gizmo['category']; ?>">
	    </tr>
	<? endif; ?>
...
?>
	    <tr id="text02">
	     ...
	     <td colspan="3" width="90%"><textarea rows="25" style="width: 100%" name="text"><?= $session->gizmo['text']; ?></textarea></td>
	    </tr>
<?php
            if ($config->use_wackoformatter) {
...
?>
	...
	<? if ($config->use_url_forms): ?>
	    ...
	    <tr id="addons01" <? if (empty($session->gizmo['url1']) &&  empty($session->gizmo['url_name1']) && empty($session->gizmo['url2']) &&  empty($session->gizmo['url_name2'])): ?> style="display: none; "<? endif; ?>>
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->url; ?>:</strong></td>
	     <td width="40%"><input style="width: 100%" type="text" name="url1" value="<?= $session->gizmo['url1']; ?>"></td>
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->description; ?>:</strong></td>
	     <td width="40%"><input style="width: 100%" type="text" name="url_name1" value="<?= $session->gizmo['url_name1']; ?>"></td>
	    </tr>
	    <tr id="addons02" <? if (empty($session->gizmo['url1']) &&  empty($session->gizmo['url_name1']) && empty($session->gizmo['url2']) &&  empty($session->gizmo['url_name2'])): ?> style="display: none; "<? endif; ?>>
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->url; ?>:</strong></td>
	     <td width="40%"><input style="width: 100%" type="text" name="url2" value="<?= $session->gizmo['url2']; ?>"></td>
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->description; ?>:</strong></td>
	     <td width="40%"><input style="width: 100%" type="text" name="url_name2" value="<?= $session->gizmo['url_name2']; ?>"></td>
	    </tr>
	    <tr><td id="header" class="t-border" colspan="4">&nbsp;</td></tr>
        <? endif; ?>
	    ...
	    <tr><td id="header" class="b-border" colspan="4"><span class="parthead"><a onClick="hideIt('keys01'); hideIt('keys02'); "><?= $i18n->keys; ?></a></span></td></tr>
    	    <tr id="keys01">
             ...
             <td width="60%"><textarea rows="5" style="width: 100%" name="keys" <? print ((($session->security->canUser($session->module, $config->right_to_keywords) == 1) || ($session->user['isadmin']))?"":"readonly" ); ?>><?= $session->gizmo['keys']; ?></textarea></td>
