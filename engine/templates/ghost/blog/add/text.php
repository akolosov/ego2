	...
	<?php $session->plugins->execHook(cHooks::beforeShowPostForm, &$session->blog); ?>
	   <table width="90%" align="center" valign="top">
	    ...
	    <tr id="header03">
	     ...
	     <td width="90%" colspan="3"><input style="width: 100%" type="text" name="topic" value="<?= $session->data['topic']; ?>"></td>
	    </tr>
...
?>
	...
	<? if ($config->use_keywords): ?>
	    <tr><td id="header" class="b-border" colspan="4"><span class="parthead"><a onClick="hideIt('keys01'); hideIt('keys02'); "><?= $i18n->keys; ?></a></span></td></tr>
	     ...
	     <td width="60%"><textarea rows="5" style="width: 100%" name="keys" <? print ((($session->security->canUser($session->module, $config->right_to_keywords)) || ($session->user['isadmin']))?"":"readonly" ); ?>><?= $session->data['keys']; ?></textarea></td>
