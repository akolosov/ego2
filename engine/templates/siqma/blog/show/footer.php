<!-- $Id: footer.php,v 1.2 2005/01/31 09:32:22 hunter Exp $ -->
	<script>
          function Delete(url) {
	...
	<tr>
         <td class="t-border" width="100%" id="footer" align="right" valign="middle" colspan="2">
	  <span class="maintance"><? if ((($session->security->canUser($session->module, $config->right_to_change) == 1) && (trim($session->blog['author_name']) == trim($session->user['name']))) || ($session->user['isadmin'])): ?><a href="<?= $session->utils->makeHRURL("?module=".$session->module."&action=change&sub=edit&id=".$session->blog['id']); ?>">[<?= $i18n->change; ?>]</a>&nbsp;<? endif; ?>
...
<? if ((($session->security->canUser($session->module, $config->right_to_bookmark) == 1) || ($session->user['isadmin'])) && ($config->use_bookmarks)): ?><a href="<?= $session->utils->makeHRURL("?module=".$session->module."&action=bookmarks&sub=add&id=".$session->blog['id']); ?>">[<?= $i18n->tobookmarks; ?>]</a>&nbsp;<? endif; ?>
...
<!-- eof $Id: footer.php,v 1.2 2005/01/31 09:32:22 hunter Exp $ -->
