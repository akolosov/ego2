<!-- $Id: text.php,v 1.5 2005/02/10 06:09:08 hunter Exp $ -->
	<tr>
	  ...
	  <? endif; ?>
 	  <form name="post" action="<?= $session->utils->makeHRURL("?module=blog&action=show&sub=add&id=".$session->id); ?>" method="<?= $config->default_method; ?>">
	    ...
	    <tr id="header02">
	     ...
	     <td width="40%">
	      <? if ($config->use_categories): ?>
	       <select name="category" style="width: 100%" value="<?= $session->data['category']; ?>"><?= get_categories_list(true); ?></select>
	      <? endif; ?>
	     </td>
	    </tr>
...
<!-- eof $Id: text.php,v 1.5 2005/02/10 06:09:08 hunter Exp $ -->
