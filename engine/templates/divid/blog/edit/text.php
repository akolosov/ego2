<!-- $Id: text.php,v 1.5 2005/02/10 06:09:05 hunter Exp $ -->
        <tr>
	  ...
	  <? endif; ?>
 	  <form name="post" action="<?= $session->utils->makeHRURL("?module=blog&action=show&sub=changed&id=".$session->id); ?>" method="POST">
	   ...
	   <table width="90%" align="center" valign="top">
	    ...
	    <tr id="header01" style="display: none; ">
	     ...
	     <td width="40%"><input style="width: 100%" type="text" name="name" value="<?= $session->blog['author_name']; ?>"></td>
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->email; ?>:</strong></td>
	     <td width="40%"><input style="width: 100%" type="text" name="email" value="<?= $session->blog['author_email']; ?>"></td>
	    </tr>
	    <tr id="header02" style="display: none; ">
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->subject; ?>:</strong></td>
	     <td width="40%"><input style="width: 100%" type="text" name="topic" value="<?= $session->blog['title']; ?>"></td>
	     <td width="10%" align="right" valign="top"><strong><? if ($config->use_categories) { print $i18n->category.":"; } ?></strong></td>
	     <td width="40%">
              <? if ($config->use_categories): ?>
	       <select name="category" style="width: 100%" value="<?= $session->blog['category']; ?>"><?= get_categories_list(true); ?></select>
	      <? endif; ?>
	     </td>
...
?>
	...
	<? if ($config->use_url_forms): ?>
	    ...
	    <tr id="control01">
	     <td width="10%"> </td>
	     <td width="100%" align="right" colspan="3" align="center"><input type="submit" name="submit" style="width: 100%" value="<?= $i18n->submit; ?>"></td>
	    </tr>
	    <tr><td id="header" class="t-border" colspan="4">&nbsp;</td></tr>
	   </table>
	  </form>
...
<!-- eof $Id: text.php,v 1.5 2005/02/10 06:09:05 hunter Exp $ -->
