<!-- $Id: text.php,v 1.6 2005/04/26 07:09:44 hunter Exp $ -->
        <tr>
	  ...
	  <? endif; ?>
 	  <form name="post" action="<?= $session->utils->makeHRURL("?module=blog&action=show&sub=add&id=".$session->id); ?>" method="POST">
	   <table width="90%" align="center" valign="top">
	    ...
	    <tr id="header02">
	     ...
	     <td width="40%"><input style="width: 100%"type="text" name="topic" value="<?= $_POST['topic']; ?>"></td>
	     ...
	     <td width="40%">
	      <? if ($config->use_categories): ?>
	       <select name="category" style="width: 100%" value="<?= $_POST['category']; ?>"><?= get_categories_list(true); ?></select>
	      <? endif; ?>
	     </td>
	    </tr>
...
?>
	    <tr id="text02">
	     ...
	     <td colspan="3" width="80%"><textarea rows="25" style="width: 100%" name="text"><?= $_POST['text']; ?></textarea></td>
	    </tr>
<?php
	    if ($config->use_wackoformatter) {
...
?>
	...
	<? if ($config->use_url_forms): ?>
	    ...
	    <tr id="urls01" style="display: none; ">
	     ...
	     <td width="40%"><input style="width: 100%" type="text" name="url1" value="<?= $_POST['url1']; ?>"></td>
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->description; ?>:</strong></td>
	     <td width="40%"><input style="width: 100%" type="text" name="url_name1" value="<?= $_POST['url_name1']; ?>"></td>
	    </tr>
	    <tr id="urls02" style="display: none; ">
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->url; ?>:</strong></td>
	     <td width="40%"><input style="width: 100%" type="text" name="url2" value="<?= $_POST['url2']; ?>"></td>
	     <td width="10%" align="right" valign="top"><strong><?= $i18n->description; ?>:</strong></td>
	     <td width="40%"><input style="width: 100%" type="text" name="url_name2" value="<?= $_POST['url_name2']; ?>"></td>
	    </tr>
	    <tr><td id="header" class="t-border" colspan="4">&nbsp;</td></tr>
	<? endif; ?>
	<? if ($config->use_keywords): ?>
	    <tr><td id="header" class="b-border" colspan="4"><span class="maintance"><a onClick="hideIt('keys01'); hideIt('keys02'); "><?= $i18n->keys; ?></a></span></td></tr>
	     ...
	     <td width="60%"><textarea rows="5" style="width: 100%" name="keys" <? print ((($session->security->canUser($session->module, $config->right_to_keywords) == 1) || ($session->user['isadmin']))?"":"readonly" ); ?>><?= $_POST['keys']; ?></textarea></td>
	     <td width="30%" align="right" valign="top" colspan="2"><select name="keylist" style="width: 98%" value="" multiple size="6" onDblClick='addKey(this.value);' ><?= get_keys_list($session->module, true); ?></select></td>
 	    </tr>
...
<!-- eof $Id: text.php,v 1.6 2005/04/26 07:09:44 hunter Exp $ -->
