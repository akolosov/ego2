...
")/sm";
  ...
  function WackoPreprocess($things)
  {
    ...
    if ($thing{0} == "~") 
     if ($thing{1} == "~") return "~~".$this->WackoPreprocess( array(0, substr_x($thing, 2)) );
    ...
    else if (preg_match("/^\%\%(.*)\%\%$/s", $thing, $matches))
    {
      ...
      if (preg_match("/^\(([^\n]+?)\)(.*)$/s", $code, $matches))
      {
        ...
        // ���� � �������� ��.
          ...
          {
            $formatter = substr_x( $matches[1], 0, $sep );
            $p = " ".substr_x( $matches[1], $sep )." ";
  ...
  function WackoMiddleprocess($things)
  {
    ...
    if ($thing{0} == "~") 
     if ($thing{1} == "~") return "~~".$this->WackoMiddleprocess( array(0,substr_x($thing,2)) );
    ...
    {
      return "\xa5\xa5<div class=\"right\">".preg_replace_callback($this->LONGREGEXP, $callback, $matches[1])."</div>\xa5\xa5";
    }
    else if (preg_match("/^<<(.*)<<$/s", $thing, $matches))
    {
      return "\xa5\xa5<div class=\"left\">".preg_replace_callback($this->LONGREGEXP, $callback, $matches[1])."</div>\xa5\xa5";
    }
    // centered text
    else if (preg_match("/^>>(.*)<<$/s", $thing, $matches))
    {
  ...
  function WackoCallback($things)
  {
    ...
    if (!$this->page_id) $this->page_id = trim(substr_x(crc32(time()),0,5),"-");
    ...
    else if (preg_match("/^\|\|(.*?)\|\|$/s", $thing, $matches) && $this->tableScope)
    {
      ...
      if (($this->cols <> 0) and ($count<$this->cols))
      {
        $this->tdoldIndentLevel = 0;
        $this->tdindentClosers = array();
        if ($cells[$i]{0}=="\n") $cells[$count] = substr_x($cells[$count], 1);
    ...
    // urls
      ...
      if (substr_x($url, -4) == ".jpg" || substr_x($url, -4) == ".gif" || substr_x($url, -4)== ".png" || substr_x($url, -4)== ".jpe"
      || substr_x($url, -5) == ".jpeg") return "<img src=\"".$matches[1]."\" />".$matches[2];
             ...
             (preg_match("/^\(\((\S+)(\s+(.+))?\)\)$/", $thing, $matches)))
    {
      ...
      if ($url{0}=="*")
      {
       ...
       else if (preg_match("/^\*\d+$/", $url)) $aname = "ftnd".substr_x($url, 1);
       else {
        $aname = htmlspecialchars(substr_x($url, 1));
        $sup = 0;
       }
       if (!$text) $text = substr_x($url, 1);
       return ($sup?"<sup>":"")."<a href=\"#o".$aname."\" name=\"".$aname."\">".$text."</a>".($sup?"</sup>":"");
      }
      else if ($url{0}=="#")
      {
       $anchor = substr_x($url, 1);
       $sup = 1;
      ...
      else
      {
       ...
       if ($url{0}=="(") {$url=substr_x($url,1); $result.="(";}
       if ($url{0}=="[") {$url=substr_x($url,1); $result.="[";}
       if (!$text) $text = $url;
       $url = str_replace( " ", "", $url );
    ...
    if (($thing[0] == "~") && ($thing[1] == "~")) return "~".preg_replace_callback($this->LONGREGEXP, $callback, substr_x($thing,2));
    // if we reach this point, it must have been an accident.
  ...
  function PostCallback($things)
  {
    ...
    else if (preg_match("/^\xA1\xA1\s*([^\n]+?)\xA1\xA1$/s", $thing, $matches))
    {
      ...
      // ���� � �����
        ...
        {
          $action = substr_x( $matches[1], 0, $sep );
          $p = " ".substr_x( $matches[1], $sep )." ";
          $paramcount = preg_match_all( "/(([^\s=]+)(\=((\"(.*?)\")|([^\"\s]+)))?)\s/", $p, 
                                        $matches, PREG_SET_ORDER );
          $params = array();  $c=0;
